...
def build_active_storage_blob(instance, attachment)
  ...
  blob_values = [blob_key, filename, content_type, service_name, file_size, file_checksum, created_at]
...
def build_active_storage_attachment(instance, attachment, model)
  ...
  attachment_values = [blob_name, record_type, record_id, max_blob_id, created_at]
...
def max_blob_id
  ActiveRecord::Base.connection.execute('SELECT MAX(id) FROM active_storage_blobs').first["max"]
end
